...
- [x] **UEFI boot — DONE + OVMF-VERIFIED (2026-07-03)** — didn't need the modern-live-build migration after all: `.github/workflows/uefi-remaster.yml` post-processes the BIOS ISO into a hybrid BIOS+UEFI image (inject an EFI El Torito boot image via `grub-mkstandalone` with an embedded menu that `search --file /casper/vmlinuz`; repack with `xorriso -as mkisofs -isohybrid-mbr isohdpfx.bin -eltorito-alt-boot -e EFI/boot/efiboot.img -isohybrid-gpt-basdat`), then **boots it under real UEFI firmware (OVMF) in QEMU → reached systemd/userspace, PASS**. TODO: fold the remaster into build.sh so every published ISO is hybrid.
