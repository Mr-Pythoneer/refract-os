...
- [~] **Functional half CI-verified on CPU (2026-07-04, `ai-serve-smoke.yml` Job B)** — the official headless LM Studio daemon (`llmster`, `lmstudio/llmster-preview:cpu`) installs and its `lms` CLI serves the OpenAI API **headless on CPU** (daemon up in 3s, `/v1/models` responds) — no GUI/Xvfb/FUSE. This is the "installs + `lms` CLI works" clause. **Still (needs hardware)**: each catalog model pulling + loading with correct `--gpu` offload + real tokens/sec (a runner has no GPU; the on-CPU model pull is a time-boxed best-effort bonus in the same job).
- [ ] **(needs hardware)** Llama-3.3-70B partial-offload (~6-12 tok/s), vision mmproj loads, ComfyUI sees the 5090, FLUX/SDXL render
- [~] **Serve+thin-client contract CI-verified on CPU (2026-07-04, `ai-serve-smoke.yml` Job A)** — a tiny GGUF served on :8080 (llama.cpp) + the repo's `distro-ai-ask` thin client returns a real completion, proving the exact `/v1/chat/completions` contract the overlay / Nautilus action / AI mode all use. The `switch ai` model-autoload plumbing is separately unit-tested (`test_modectl.sh`). **Still (needs hardware)**: the real use-case model auto-loading on a GPU box and thin clients answering from *that*.
...
- [x] CI now EXECUTES scripts, not just lints: `.github/workflows/tests.yml` runs `tests/run.sh` on `ubuntu-latest` — a stub-based suite (9 files, **171 assertions** as of 2026-07-04, +6 for the intel_pstate governor mapping) for the pure-logic scripts (modectl, gaming-compat, creative-scratch, ai-ask, cloud-toggle, detect-tier, ai-model, ai-setup) + a compat-db schema validator. The CI run does what a macOS box can't: e.g. asserts the `df -l --output` form works on real GNU coreutils. See `tests/README.md`.
- [x] **Two behavioral CI workflows added (2026-07-04)** that exercise real integration surfaces on the runner (itself a real Ubuntu systemd VM), not just stubs: `server-ssh-survival.yml` (installs real gdm+sshd, proves `switch server` doesn't drop a live SSH session — see §6 line 82) and `ai-serve-smoke.yml` (Job A: llama.cpp serves a tiny GGUF on :8080 + `distro-ai-ask` gets a real answer, proving the OpenAI serve/thin-client contract; Job B: the official headless LM Studio `llmster` CPU daemon installs + `lms` CLI serves — see §3 lines 44/46). Both dispatch-only.
